...
export default function AuthorizationCard() {
  ...
  const submitBypass = async (selectedUser: string, overridePass?: string) => {
    const finalPass = overridePass || password.trim();
    setIsLoading(true);
    setError(null);
    try {
  ...
  const handleBypassSubmit = async (e: React.FormEvent) => {
    ...
    await submitBypass(trimmedUser, trimmedPass);
  };
      ...
      >
        ...
        {showAdminSelector && (
          <div className="w-full flex flex-col gap-4 animate-fadeIn">
            <p className="font-rajdhani text-xs text-terminal-green tracking-wider uppercase text-center">
              Select Operator Profile
            </p>
            <div className="flex flex-col gap-3 w-full">
              <button
                type="button"
                onClick={() => submitBypass('aruneshwarank')}
                disabled={isLoading}
                className="w-full py-3 bg-terminal-green/5 border border-terminal-green/30 text-terminal-green hover:bg-terminal-green/10 transition-all rounded-lg font-mono text-xs font-semibold tracking-wider uppercase disabled:opacity-50"
              >
                [ Aruneshwaran ]
              </button>
              <button
                type="button"
                onClick={() => submitBypass('harigh')}
                disabled={isLoading}
                className="w-full py-3 bg-arc-blue/5 border border-arc-blue/30 text-arc-blue hover:bg-arc-blue/10 transition-all rounded-lg font-mono text-xs font-semibold tracking-wider uppercase disabled:opacity-50"
              >
                [ Harish ]
              </button>
            </div>
            <button
              ...
              disabled={isLoading}
              className="text-center font-mono text-[9px] text-white/40 hover:text-white/60 transition-colors uppercase tracking-widest mt-2"
            >
              &lt; Back to Credentials
            </button>
          </div>
